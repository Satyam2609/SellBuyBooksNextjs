...
export default function EnginerringBook() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      console.log("EnginerringBook: Calling API at", `${process.env.NEXT_PUBLIC_BOOK_URL}/api/engineeringBook`);
      try {
        const res = await axios.get(`${process.env.NEXT_PUBLIC_BOOK_URL}/api/engineeringBook`, {
          withCredentials: true,
        });
        console.log("EnginerringBook: API Response:", res.data);
        setBooksData(res.data.findata || []);
      } catch (error) {
        console.log("EnginerringBook: Fetch Error:", error.response?.data?.message || error.message);
      }
    };
    fetchData();
  }, []);
  ...
  return (
    <section className="bg-gray-50 py-14">
      <div className="max-w-8xl mx-auto px-4 sm:px-6">
            ...
            >
              ...
              <div className="p-4">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${book.condition === "New"
        ...
        {/* TOAST */}
        <div
          className={`fixed bottom-5 right-5 z-50 transition ${popup ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
            }`}
        >
